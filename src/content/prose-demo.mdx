...
styled through the `--vr*` token namespace.
...
```typescript
// Wiring the prose token namespace for the vr demo
const proseTokens = {
  vspMd:          `var(--vr-vsp-md)`,
  textBody:       `var(--vr-text-body)`,
  leadingRelaxed: `var(--vr-leading-relaxed)`,
  codeBg:         `var(--vr-code-bg)`,
  codeFg:         `var(--vr-code-fg)`,
  fontMono:       `var(--vr-font-mono)`,
} as const;
...
1. Define the token set in `--vr-*` namespace.
...
Heading borders and blockquote accents reuse the shared `--vr-color-fg`,
